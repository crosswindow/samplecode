...
static void gpio_motion_keys_report_event(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	state = (__gpio_get_value(s_ctrl->motion_dev.gpio_motion) ? 1 : 0);
	pr_info("gpio state: %d key_code:0X%x\n", state, s_ctrl->motion_dev.key_code);
	input_report_key(s_ctrl->motion_dev.motion_input, s_ctrl->motion_dev.key_code, state);
	input_sync(s_ctrl->motion_dev.motion_input);
}
...
static void gpio_motion_camera_work_func(struct work_struct *work)
{
	...
	if (s_ctrl->motion_dev.motion_interrupt_mode)
	{
...
/*		if (s_ctrl->motion_dev.is_start_camera) {
			gpio_motion_keys_report_event(s_ctrl);
		}*/
	}
...
static int motion_interrupt_setup(struct platform_device *pdev, struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = of_property_read_u32(of_node, "motion,key-code",
		&s_ctrl->motion_dev.key_code);
	pr_err("qcom, motion key code = %d, rc %d", s_ctrl->motion_dev.key_code, rc);
	if (rc < 0) {
		s_ctrl->motion_dev.key_code = 0x2fe;
		rc = 0;
	}
	...
	if (gpio_is_valid(s_ctrl->motion_dev.gpio_motion)) {
		...
		error = devm_request_irq(dev, s_ctrl->motion_dev.irq_motion,
	...
	if (!s_ctrl->motion_dev.motion_input) {
		s_ctrl->motion_dev.motion_input = input_allocate_device();
		if (!s_ctrl->motion_dev.motion_input) {
			dev_err(dev,
				"Can't allocate motion input device\n");
			return -ENOMEM;
		}
		s_ctrl->motion_dev.motion_input->name = "montion_sensor";
		s_ctrl->motion_dev.motion_input->phys = "montion_sensor/input4";
		s_ctrl->motion_dev.motion_input->id.bustype = BUS_HOST;
		s_ctrl->motion_dev.motion_input->id.vendor = 0x0011;
		s_ctrl->motion_dev.motion_input->id.product = 0x0011;
		s_ctrl->motion_dev.motion_input->id.version = 0x0100;
	}
	input_set_capability(s_ctrl->motion_dev.motion_input, EV_KEY, s_ctrl->motion_dev.key_code);
	rc = input_register_device(s_ctrl->motion_dev.motion_input);
	if (rc) {
		if (s_ctrl->motion_dev.motion_input) {
			input_free_device(s_ctrl->motion_dev.motion_input);
			s_ctrl->motion_dev.motion_input = NULL;
		}

		dev_err(dev,
			"Can't register montion sensor key: %d\n", rc);
		return rc;
	}
